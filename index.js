...
const translateMessage = async (chatId, text) => {
	...
	if (!text)
		return bot.sendMessage(chatId, 'סליחה, לא ציינת את הטקסט. כתוב אותו אחרי הפקודה /translate באותה הודעה\nSorry, you didn\'t specify the text. Write it after the /translate command in the same message\nИзвините, вы не указали текст. Напишите его после команды /translate в том же сообщении\nDésolé, vous n\'avez pas précisé le texte. Écrivez-le après la commande /translate dans le même message')
...
const start = async ({ chat }) => await bot.sendMessage(chat.id, 'שלום, אני בוט של הקבוצה הטובה ביותר באולפן\n\nHello, I am a bot from the best group in the ulpan.\n\nПривет, я бот самой лучшей группы в ульпане.\n\nSalut, je suis un bot du meilleur groupe à l\'oulpan.')
...
const init = () => {
	bot.setMyCommands([
		...
		{ command: '/sethomework', description: 'Установить домашнее задание' },
		{ command: '/gethomework', description: 'Проверить домашнее задание' },
		{ command: '/timetable', description: 'Проверить расписание' },
		{ command: '/translate', description: 'Перевести сообщение' }
	])
